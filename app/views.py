    ...
    if session['user']:
        ...
        if request.method == 'POST':
            ...
            if rename == '':
                ...
                return redirect (f'/dashboard/rename/{dname}', dname=dname)
                ...
                if os.path.exists(dash_file):
                    ...
                    for pv in updated_readings_from_jaya['readPvDict']:
                        if isinstance(updated_readings_from_jaya['readPvDict'][pv], str) \
                                ...
                                and not "\"" in updated_readings_from_jaya['readPvDict'][pv]:
                            updated_readings_from_jaya['readPvDict'].update({pv : f"{float(updated_readings_from_jaya['readPvDict'][pv]):.3f}"})
